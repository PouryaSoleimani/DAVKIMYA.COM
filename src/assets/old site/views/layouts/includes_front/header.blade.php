    ...
    <div class="border-bottom">
        <div
            ...
            <div class="d-flex flex-wrap align-items-center justify-content-center gap-md-5 gap-3">
                ...
                {{--                      </svg>--}}
                {{--                      <span class="secondary-color">{{ read_lang($contact_info, 'address') }}</span>--}}
                {{--                  </div>--}}
                @if (!blank($contact_info->phone) && explode(',', $contact_info->phone) > 0)
                    @foreach(explode(',', $contact_info->phone) as $key=>$phone)
                        @if($key<2)
                            <div class="d-flex align-items-center gap-2">
                                <svg xmlns="http://www.w3.org/2000/svg" width="19" height="19" fill="currentColor"
                ...
                @if (!blank($contact_info->email) && explode(',', $contact_info->email) > 0)
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <span class="secondary-color">{{ explode(',', $contact_info->email)[0] }}</span>
                    </div>
                @endif
            </div>
            <!-- socials -->
            @if (
                $contact_info->telegram ||
                    ...
                    $contact_info->whatsapp_group)
                <div class="d-flex align-items-center gap-2">

                    @foreach(menu_langs() as $index=>$lang)
                        <a href="{{ LaravelLocalization::getLocalizedURL($lang->lang, null, [], true) }}"
                           class="d-block primary-bg transition rounded-circle p-2 d-flex align-items-center justify-content-center">
                            <img src="{{url($lang->photo->path)}}" class="lang_menu" alt="{{$lang->lang}}" width="30px">
                        </a>
                    @endforeach
        ...
        <nav class="navbar navbar-expand-lg bg-body-tertiary container">
            <div class="container-fluid">
                ...
                <a class="navbar-brand" href="{{ route('front.index') }}">
                    <img src="{{ url($logo) }}" alt="{{ $titleSeo }}" class="img-fluid" width="100">
                </a>
                ...
                <div class="collapse navbar-collapse" id="navbarSupportedContent">
                    <ul class="navbar-nav ms-auto mb-2 mb-lg-0 gap-4">
                        <li class="nav-item">
                            <a class="nav-link primary-color active" aria-current="page"
                               href="{{ route('front.index') }}">{{ read_lang_word('منو', '1') }}</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link primary-color"
                               href="{{ route('front.about.us') }}">{{ read_lang_word('منو', '4') }}</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link primary-color"
                               href="{{ route('front.product.list') }}">{{ read_lang_word('منو', '9') }}</a>
                        </li>
                        {{-- <li class="nav-item">
                              <a class="nav-link primary-color" href="{{ route('front.project.list')}}">{{read_lang_word('منو','7')}}</a>
                          </li> --}}
                        <li class="nav-item">
                            <a class="nav-link primary-color"
                               href="{{ route('front.service.list') }}">{{ read_lang_word('منو', '3') }}</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link primary-color"
                               href="{{ route('front.faq') }}">{{ read_lang_word('منو', '8') }}</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link primary-color"
                               href="{{ route('front.blog.list') }}">{{ read_lang_word('منو', '2') }}</a>
                        </li>
                        {{-- <li class="nav-item dropdown">
                              ...
                              <ul class="dropdown-menu">
                                  <li><a class="dropdown-item" href="{{ route('front.faq') }}">{{read_lang_word('منو','8')}}</a></li>
                                  <li><a class="dropdown-item" href="{{ route('front.blog.list') }}">{{read_lang_word('منو','2')}}</a></li>

                              </ul>
                          </li> --}}
                        <li class="nav-item">
                            <a class="nav-link primary-color"
                               href="{{ route('front.contact.us') }}">{{ read_lang_word('منو', '5') }}</a>
                        </li>
                    </ul>
